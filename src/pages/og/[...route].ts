...
export const { getStaticPaths, GET } = await OGImageRoute({
  ...
  getImageOptions: (_path, page) => ({
    ...
    logo: undefined,
    // Fallback gradient (shown only if the bg image fails to load).
    bgGradient: [
      [17, 13, 22], // --bg
      [12, 9, 19], // --bg-deep
    ],
    // Striking exhibition-print background with a baked-in dark scrim for
    // text legibility. Generated by scripts/gen-og-bg.mjs.
    bgImage: { path: './src/assets/og-bg.jpg', fit: 'cover' },
    border: { color: [181, 133, 232], width: 12, side: 'inline-start' }, // --c3
    padding: 80,
    font: {
      title: {
        color: [245, 238, 226], // near --fg, lifted for contrast over imagery
        size: 72,
        weight: 'Bold',
      ...
      description: {
        color: [214, 208, 197], // lifted from --fg-2 for contrast over imagery
        size: 30,
        lineHeight: 1.4,
