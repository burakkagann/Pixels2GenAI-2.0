...
**Why does this produce cells?** Local activation (edge amplification by `Math`) plus lateral inhibition (dampening by `Level` and blur) plus temporal recursion equals Turing's reaction-diffusion mechanism from 1952 — lesson 5.4.4 Turing Patterns (coming soon) covers the chemistry version. You have just built a real-time analogue computer for morphogenesis using ten node graphs and zero Python.
